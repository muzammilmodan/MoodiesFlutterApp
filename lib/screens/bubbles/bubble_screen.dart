  ...
  void _loadGame() {
    ...
    bubbles = List.generate(
      ...
      (index) => BubbleState(
        ...
        number: (rule?.contains('N') ?? false) ? index + 1 : null,
      ),
    );
    ...
    bubbles.forEach(
      (item) => colours.values.elementAt(item.colorIndex ?? 0).incrementCount(),
    );
    ...
    switch (rule) {
      ...
      case 'N':
        ruleCount = (bubbles.length / (ruleNumber ?? 0)).floor();
        break;
      ...
      case 'NC':
        ruleCount = bubbles
            .where((element) =>
                ...
                (element.number ?? 0) % (ruleNumber ?? 0) == 0)
            .length;
        break;
    }
